...
import sys
sys.path.insert(1, '../..')
from HCM.util.preprocessing import read_variable_to_vector, read_multiple_varible_to_array
from HCM.util.preprocessing import nan_read_multiple_variable_temporal_to_vector
from HCM.util.preprocessing import read_multiple_variable_temporal_to_vector
from HCM.util.preprocessing import generate_data_loader, generate_input_loader

from HCM.optimization.forward import embed_prediction_forward, parameter_prediction_forward
from HCM.optimization.loss_functions import *

from HCM.model.DALEC993 import DALEC993
import argparse
...
if "jianingfang" in os.getcwd():
    CARDAMOM_DRIVER_DATA_DIR = "/Users/jianingfang/Dropbox/Columbia/HybridModeling/CARDAMOM_driver_data"
elif "burg" in os.getcwd():
...
parser.add_argument('-c', '--cms', default="true")
parser.add_argument('-l', '--length')
parser.add_argument('-s', '--shuffle', default="true")
parser.add_argument('-p', '--scatterplot', default="false")
...
output_dir = "./output/"
log_dir = "./log/"
fig_dir = "./postanalysis/figure/"
nc_dir = "./postanalysis/nc/"
npy_dir = "./postanalysis/npy/"
metrics_dir = "./postanalysis/metrics/"
...
GLOBAL_DATA_DIR = os.path.join(CARDAMOM_DRIVER_DATA_DIR, "global/")

args = parser.parse_args()

if args.experiment == "feature": 
    exp_df = pd.read_csv("../../notebooks/global/feature_selection_experiment.csv")
elif args.experiment == "standard":
    exp_df = pd.read_csv("../../notebooks/global/standard_experiment.csv")
elif args.experiment == "clim_soil_age":
    exp_df = pd.read_csv("../../notebooks/global/clim_soil_age_experiment.csv")
elif args.experiment == "control":
    exp_df = pd.read_csv("../../notebooks/global/control_experiment.csv")
elif args.experiment == "sensitivity":
    exp_df = pd.read_csv("../../notebooks/global/sensitivity_experiment.csv")

run = int(exp_df.loc[IDX, "run"])
...
def get_predictor_list(IDX, exp_df):
    predictor_list = ["LAT"]
    if "PFT" in exp_df.loc[IDX, "predictor_set"]:
        predictor_list += ["NF", "DBF", "EBF", "MF", "SH", "SAV", "GRA", "WET", "CRO", "NVG"]
    if "CLIM" in exp_df.loc[IDX, "predictor_set"]:
        predictor_list += ["MAT", "MAP", "elevation"]
    if "AGE" in exp_df.loc[IDX, "predictor_set"]:
        predictor_list += ["canopy_height", "tree_age_000_filled"]
    if "SOIL" in exp_df.loc[IDX, "predictor_set"]:
        predictor_list += ["BULK_DEN", "SAND", "SILT", "CLAY", "GRAVEL"]
    if "LATLON" in exp_df.loc[IDX, "predictor_set"]:
        predictor_list += ["lat_deg", "lon_deg"]
    if "CONTROL" in exp_df.loc[IDX, "predictor_set"]:
        predictor_list += ["null"]
    return predictor_list

predictor_list = get_predictor_list(IDX, exp_df)

exp_str = "dalec993_{}_threshold_{}_run_{}".format(exp_df.loc[IDX, "predictor_set"],
                                exp_df.loc[IDX, "threshold_dim"],
                                exp_df.loc[IDX, "run"])
if args.fluxnet == "true":
    if args.shuffle == "true":
        exp_str += "_fluxnet"
    else:
        exp_str += "_fluxnetnoshuffle"
else:
    if args.cms != "true":
        exp_str += "_no_cms"
...
def compute_patch_level_nee(model_name, predictor_loader, met_loader, label_loader, model, warm_up=2*12):
    with open(model_name, "rb") as fp:
        param_state = pickle.load(fp)
    ...
    for predictor, met, label in zip(predictor_loader, met_loader, label_loader):
        ...
        nee_predicted.append(np.sum(predicted_sample[:, warm_up:, model.pfn.nbe] * label[:, warm_up:, 3], axis=0) / np.sum(label[:, warm_up:, 3], axis=0))
...
if args.shuffle == "true":
    RUN_SIMULATION_IDX = read_variable_to_vector(GLOBAL_DATA_DIR, "run_simulation_idx_v6.nc", "run_simulation_idx", time_idx=run-1)
    
else:
    RUN_SIMULATION_IDX = read_variable_to_vector(GLOBAL_DATA_DIR, "run_simulation_idx_v6.nc", "run_simulation_idx", time_idx=60)

VALID = read_variable_to_vector(GLOBAL_DATA_DIR, "era_valid_v6.nc", "era_valid")
...
predictor_matrix = read_multiple_varible_to_array(GLOBAL_DATA_DIR, "combined_global_initial_v6.nc", predictor_list)
...
ASSIMILATE_BULK_FLAG = read_variable_to_vector(GLOBAL_DATA_DIR, "assimilate_bulk_variable_v6.nc", "assimilate_bulk_variable")
...
if args.shuffle == "true":
    ...
    train_dev_idx = np.round(np.max(sorted_valid_idx) * 1.0).astype(np.int32)
...
met_matrix = read_multiple_variable_temporal_to_vector(GLOBAL_DATA_DIR, "combined_global_initial_v6.nc", met_list, not_nan_idx, shuffle_idx, n_t=NT)
...
test_met_matrix = read_multiple_variable_temporal_to_vector(GLOBAL_DATA_DIR, "combined_global_initial_v6.nc", met_list, test_not_nan_idx, test_shuffle_idx, n_t=NT)
...
output_matrix = nan_read_multiple_variable_temporal_to_vector(GLOBAL_DATA_DIR, "combined_global_initial_v6.nc", output_list, not_nan_idx, shuffle_idx, n_t=NT)
...
output_matrix_test = nan_read_multiple_variable_temporal_to_vector(GLOBAL_DATA_DIR, "combined_global_initial_v6.nc", output_list, test_not_nan_idx, test_shuffle_idx, n_t=NT)
...
pickle_name = os.path.join("../../experiments/global/{}/".format(output_dir), exp_str + ".pickle")
...
for k in range(len(train_X)):
    ...
    nee_predicted_train.append(np.sum(predicted_sample[:, warm_up:, model.pfn.nbe] * train_Y[k][:, warm_up:, 3], axis=0) / np.sum(train_Y[k][:, warm_up:, 3], axis=0))
...
for k in range(len(test_X)):
    ...
    nee_predicted_test.append(np.sum(predicted_sample[:, warm_up:, model.pfn.nbe] * test_Y[k][:, warm_up:, 3], axis=0) / np.sum(test_Y[k][:, warm_up:, 3], axis=0))
...
if args.shuffle=="true":
    ...
    for k in range(len(dev_X)):
        ...
        nee_predicted_dev.append(np.sum(predicted_sample[:, warm_up:, model.pfn.nbe] * dev_Y[k][:, warm_up:, 3], axis=0) / np.sum(dev_Y[k][:, warm_up:, 3], axis=0))
...
with open(os.path.join(metrics_dir, "{}_metrics.json".format(exp_str)), "w") as fp:
    json.dump(str(result_dict), fp, indent=4)
...
predictor_matrix_all = read_multiple_varible_to_array(GLOBAL_DATA_DIR, "combined_global_initial_v6.nc", predictor_list)
...
met_list = ["DAYS", "T_min", "T_max", "SOLR", "CO2", "DOY", "BURNED_AREA", "VPD", "PREC", "LAT", "DELTA_T", "MAT", "MAP"]
met_matrix_all = read_multiple_variable_temporal_to_vector(GLOBAL_DATA_DIR, "combined_global_initial_v6.nc", met_list, np.full(predictor_matrix_all.shape[1], True), np.arange(0, predictor_matrix_all.shape[1]), n_t=NT)
...
observed_lai_da = xr.open_dataset(os.path.join(GLOBAL_DATA_DIR, "combined_global_initial_v6.nc"))["LAI"][warm_up:, :, :]
...
observed_sif_da = xr.open_dataset(os.path.join(GLOBAL_DATA_DIR, "combined_global_initial_v6.nc"))["SIF"][warm_up:, :, :]
...
observed_et_da = xr.open_dataset(os.path.join(GLOBAL_DATA_DIR, "combined_global_initial_v6.nc"))["ET"][warm_up:, :, :]
...
observed_nbe_da = xr.open_dataset(os.path.join(GLOBAL_DATA_DIR, "combined_global_initial_v6.nc"))["NBE"][warm_up:, :, :]
...
observed_ewt_da = xr.open_dataset(os.path.join(GLOBAL_DATA_DIR, "combined_global_initial_v6.nc"))["LWE_normalized"][warm_up:, :, :]
predicted_ewt_da = xr.DataArray(predicted_all[:, :, warm_up:, model.pfn.next_water_pool].transpose([2,0,1]), coords={"time":observed_ewt_da.time,
                                                                                "lat":np.linspace(89.875, -89.875, 720), 
                                        "lon":np.linspace(-179.875, 179.875, 1440)})
observed_ewt_da.values = coarsen_numpy(observed_ewt_da.values)
predicted_ewt_da.values = coarsen_numpy(predicted_ewt_da.values)
...
ax[0].imshow(cor_lai_annual, transform=ccrs.PlateCarree(),
        aspect='auto', extent=[-180,180,-90,90],
        interpolation="none", cmap=ccmap, vmin=-1, vmax=1)
...
ax[1].imshow(cor_sif_annual, transform=ccrs.PlateCarree(),
        aspect='auto', extent=[-180,180,-90,90],
        interpolation="none", cmap=ccmap, vmin=-1, vmax=1)
...
ax[2].imshow(cor_nbe_annual, transform=ccrs.PlateCarree(),
        aspect='auto', extent=[-180,180,-90,90],
        interpolation="none", cmap=ccmap, vmin=-1, vmax=1)
...
ax[3].imshow(cor_ewt_annual, transform=ccrs.PlateCarree(),
        aspect='auto', extent=[-180,180,-90,90],
        interpolation="none", cmap=ccmap, vmin=-1, vmax=1)
...
ax[4].imshow(cor_et_annual, transform=ccrs.PlateCarree(),
        aspect='auto', extent=[-180,180,-90,90],
        interpolation="none", cmap=ccmap, vmin=-1, vmax=1)
...
ax[5].imshow(cor_vod_annual, transform=ccrs.PlateCarree(),
        aspect='auto', extent=[-180,180,-90,90],
        interpolation="none", cmap=ccmap, vmin=-1, vmax=1)
...
plt.savefig(os.path.join(fig_dir, "{}_spatial_r_val_annual.png".format(exp_str)), dpi=300)
"""
...
if exp_df.loc[IDX, "predictor_set"] in ["PFT", "PFT+CLIM+SOIL+AGE", "CLIM+SOIL+AGE"]:
    metrics_ds = xr.Dataset()
    ...
    metrics_ds["ewt_initial"] = predicted_ewt_da[0, :, :].expand_dims(dim={"run":[run,]}, axis=0)
    metrics_ds["ewt_final"] = predicted_ewt_da[-12 :, :].expand_dims(dim={"run":[run,]}, axis=0)
    ...
    metrics_ds.to_netcdf(os.path.join("./postanalysis/nc/", "{}_mean_std.nc".format(exp_str)))
    
    ewt_ds = xr.Dataset({"predicted_ewt":predicted_ewt_da.astype(np.float32).expand_dims(dim={"run":[run,]}, axis=0)})
    ewt_ds.to_netcdf(os.path.join("./postanalysis/nc/", "{}_ewt.nc".format(exp_str)))

    nbe_ds = xr.Dataset({"predicted_nbe":predicted_nbe_da.expand_dims(dim={"run":[run,]}, axis=0)})
    nbe_ds.to_netcdf(os.path.join("./postanalysis/nc/", "{}_nbe.nc".format(exp_str)))
    ...
    gpp_et_ds.to_netcdf(os.path.join("./postanalysis/nc/", "{}_gpp_et.nc".format(exp_str)))
