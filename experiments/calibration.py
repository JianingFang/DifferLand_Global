...
logging.info("Number of hidden layers in the embeeding NN: {}".format(HIDDEN_LAYERS))
logging.info("Number of neurons in each NN layer: {}".format(NEURONS))
logging.info("Learning rate: {}".format(LEARNING_RATE))
logging.info("Number of timesteps: {}".format(NT))
logging.info("Spatsial predictors:")
for p in predictor_list:
    logging.info("\t+ {}".format(p))
logging.info("Targer variables:")
logging.info(f"\t* {SIF_PROXY_TARGET=}")
logging.info(f"\t* {NBE_INVERSION_TARGET=}")
logging.info(f"\t* {SATELLITE_LAI_TARGET=}")
logging.info(f"\t* {GRACE_LWE_TARGET=}")
logging.info(f"\t* {BIOMASS_TARGET=}")
logging.info(f"\t* {FIRE_EMISSION_TARGET=}")
logging.info(f"\t* {STATIC_SOIL_TARGET=}")
logging.info(f"\t* {VOD_TARGET=}")
logging.info(f"\t* {GPP_FLUXNET_TARGET=}")
logging.info(f"\t* {RECO_FLUXNET_TARGET=}")
logging.info(f"\t* {ET_FLUXNET_TARGET=}")
logging.info(f"\t* {GLEAM_ET_TARGET=}")
