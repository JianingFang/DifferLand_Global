...
CARDAMOM_DRIVER_DATA_DIR = (
    "/burg-archive/glab/users/jf3423/data/CARDAMOM_driver_data/global/"
)
...
if args.verbose:
    ...
    print("Warm up period excluded from evaluation: {}".format(warm_up))
...
def safe_mean(x, axis=None):
    """Safe mean: return NaN if slice is empty."""
    if x.size == 0:
        return jnp.nan
    return jnp.mean(x, axis=axis)


def safe_prod(x, axis=None):
    """Safe product: return NaN if slice is empty."""
    if x.size == 0:
        return jnp.nan
...
def nse_score(targets, predictions):
    return 1 - (
        np.sum((targets - predictions) ** 2) / np.sum((targets - np.mean(targets)) ** 2)
    )
...
def weighted_mean_over_batch(x_px_t, w_px_t, axis=0):
    ...
    Inputs: x_px_t, w_px_t with shape (pixels, time) (or broadcastable).
    Returns: (time,) — sum_over_pixels(x*w) / sum_over_pixels(w)
    """
    if x_px_t.size == 0 or w_px_t.size == 0:
        return jnp.nan
    num = jnp.sum(x_px_t * w_px_t, axis=axis)
    den = jnp.sum(w_px_t, axis=axis) + EPS
    return num / den


def center_by_time_weighted_per_pixel(pred_px_t, w_px_t):
    """
    pred_px_t: (pixels, time) scalar variable per pixel across time
    w_px_t:    (pixels, time) weights across time per pixel
    Returns pred centered by each pixel's **time-weighted** mean.
    """
    if pred_px_t.size == 0:
        return pred_px_t
    # pixel-wise time-weighted mean -> (pixels,)
    mean_per_pixel = jnp.sum(pred_px_t * w_px_t, axis=1) / (
        jnp.sum(w_px_t, axis=1) + 0.1
    )
    return pred_px_t - mean_per_pixel[:, None]


def mean_over_pixels_with_time_weights(x_px_t, w_px_t):
    """
    Mean over pixels of (x * w) at each time step.
    Returns: (time,)
    """
    if x_px_t.size == 0:
        return jnp.nan
    return jnp.mean(x_px_t * w_px_t, axis=0)


def mask_time_by_threshold(x_t, w_px_t, thr):
    """
    Mask a time series x_t where the **pixel-mean** weight at that time < thr.
    x_t:    (time,)
    w_px_t: (pixels, time)
    """
    w_mean_time = safe_mean(w_px_t, axis=0)  # (time,)
    return jnp.where(w_mean_time < thr, jnp.nan, x_t)


# ---------- core split processor ----------


def process_split(param_state, X, Y, MET, warm_up, model, water_thr=0.7):
    """
    Process one split (train/test/dev), returning stacked arrays:
      nbe_pred, nbe_lab, water_pred, water_lab, fire_pred, fire_lab
    Shapes follow the originals. Axis 0 = pixels (batch), axis 1 = time.
    """
    nbe_pred, nbe_lab = [], []
    water_pred, water_lab = [], []
    fire_pred, fire_lab = [], []

    idx_nbe = model.pfn.nbe
    idx_next_water = model.pfn.next_water_pool
    idx_fire = model.pfn.total_fire_combust

    for k in range(len(X)):
        pred = batch_forward(param_state, X[k], MET[k]).squeeze()

        # Slices (pixels, time, var) -> select post-warmup time and weight channels
        w_nbe = Y[k][:, warm_up:, 3]  # (pixels, time)
        lab_nbe = Y[k][:, warm_up:, 2]  # (pixels, time)
        w_water = Y[k][:, warm_up:, 7]  # (pixels, time)
        lab_water_px = Y[k][:, warm_up:, 6]  # (pixels, time)
        w_fire = Y[k][:, warm_up:, 11]  # (pixels, time)
        lab_fire_px = Y[k][:, warm_up:, 10]  # (pixels, time)

        # ----- NBE: weighted mean over **batch/pixels** -> (time,)
        nbe_pred.append(
            weighted_mean_over_batch(pred[:, warm_up:, idx_nbe], w_nbe, axis=0)
        )
        nbe_lab.append(weighted_mean_over_batch(lab_nbe, w_nbe, axis=0))

        # ----- Water:
        # 1) center per pixel by its **time-weighted** mean
        pred_water_px_t = pred[:, warm_up:, idx_next_water]  # (pixels, time)
        centered_px_t = center_by_time_weighted_per_pixel(pred_water_px_t, w_water)
        # 2) mean over pixels of (centered * weights) -> (time,)
        pred_water_norm_t = mean_over_pixels_with_time_weights(centered_px_t, w_water)
        # 3) mask times with insufficient average pixel weight
        pred_water_norm_t = mask_time_by_threshold(
            pred_water_norm_t, w_water, water_thr
        )
        # labels for water (mean over pixels of label*weight), then same time mask
        lab_water_t = safe_mean(lab_water_px * w_water, axis=0)  # (time,)
        lab_water_t = mask_time_by_threshold(lab_water_t, w_water, water_thr)

        water_pred.append(pred_water_norm_t)
        water_lab.append(lab_water_t)

        # ----- Fire: weighted mean over **batch/pixels** -> (time,)
        fire_pred.append(
            weighted_mean_over_batch(pred[:, warm_up:, idx_fire], w_fire, axis=0)
        )
        fire_lab.append(weighted_mean_over_batch(lab_fire_px, w_fire, axis=0))

    # Stack across patches (k dimension) to match your downstream expectations
    return (
        np.stack(nbe_pred),
        np.stack(nbe_lab),
        np.stack(water_pred),
...
def compute_agb(predicted_matrix, output_matrix, warm_up, pfn):
    """
    Returns:
      modeled_agb_px_t, observed_agb_px_t, agb_mask_px_t
    Shapes: (pixels, time)
    """
    if predicted_matrix.size == 0 or output_matrix.size == 0:
        return jnp.nan, jnp.nan, jnp.nan
    modeled = (
        predicted_matrix[:, warm_up:, pfn.next_labile_pool]
        + predicted_matrix[:, warm_up:, pfn.next_foliar_pool]
        + predicted_matrix[:, warm_up:, pfn.next_wood_pool]
        + predicted_matrix[:, warm_up:, pfn.next_root_pool]
    )  # (pixels, time)
    observed = output_matrix[:, warm_up:, 8]  # (pixels, time)
    mask = output_matrix[:, warm_up:, 9]  # (pixels, time)
    return modeled, observed, mask


def annualize_monthly(px_t):
    """
    Aggregate monthly (pixels, time) → annual (pixels, years) via mean over 12-month blocks.
    Returns NaN if time < 12 or input empty (prevents 'mean of empty slice').
    """
    if px_t.size == 0:
        return jnp.nan
    n_px, n_t = px_t.shape
    if n_t < 12:
        return jnp.nan
    return safe_mean(px_t.reshape(n_px, -1, 12), axis=2)


def annualize_mask(mask_px_t):
    """
    Aggregate monthly mask (pixels, time) → annual (pixels, years) via product over 12-month blocks.
    Returns NaN if time < 12 or input empty.
    """
    if mask_px_t.size == 0:
        return jnp.nan
    n_px, n_t = mask_px_t.shape
    if n_t < 12:
        return jnp.nan
...
def compute_metrics(label, predicted, var_name, result_dict):
    ...
    result_dict[var_name + "_flat_nse"] = nan_filtered_r2_score(
        label_flatten[sel_flatten], predicted_flatten[sel_flatten]
    )
    result_dict[var_name + "_flat_rmse"] = nan_filtered_root_mean_squared_error(
        label_flatten[sel_flatten], predicted_flatten[sel_flatten]
    )
    result_dict[var_name + "_flat_mae"] = nan_filtered_mean_absolute_error(
        label_flatten[sel_flatten], predicted_flatten[sel_flatten]
    )
    ...
    result_dict[var_name + "_spatial_nse"] = nan_filtered_r2_score(
        label_spatial[sel_spatial], predicted_spatial[sel_spatial]
    )
    result_dict[var_name + "_spatial_rmse"] = nan_filtered_root_mean_squared_error(
        label_spatial[sel_spatial], predicted_spatial[sel_spatial]
    )
    result_dict[var_name + "_spatial_mae"] = nan_filtered_mean_absolute_error(
        label_spatial[sel_spatial], predicted_spatial[sel_spatial]
    )
    ...
    for i in range(label.shape[0]):
        s_sel = np.invert(np.isnan(label[i, :]) | np.isnan(predicted[i, :]))
        if np.sum(s_sel) > 0:
            spatial_rmse_list.append(
                root_mean_squared_error(label[i, :][s_sel], predicted[i, :][s_sel])
            )

    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=RuntimeWarning)
        result_dict[var_name + "_spatial_mean_temporal_nse"] = np.nanmean(
            spatial_r2_list
        )
        result_dict[var_name + "_spatial_median_temporal_nse"] = np.nanmedian(
            spatial_r2_list
        )
        result_dict[var_name + "_spatial_std_temporal_nse"] = np.nanstd(spatial_r2_list)

        result_dict[var_name + "_spatial_mean_temporal_rmse"] = np.nanmean(
            spatial_rmse_list
        )
        result_dict[var_name + "_spatial_median_temporal_rmse"] = np.nanmedian(
            spatial_rmse_list
        )
        result_dict[var_name + "_spatial_std_temporal_rmse"] = np.nanstd(
            spatial_rmse_list
        )
...
def plot_performance(predicted, true, ax, axis_min, axis_max, title_name=None):
    ...
    if np.sum(sel) > 0:
        ...
        ax.plot(
            predicted, res.intercept + res.slope * predicted, "red", label="fitted line"
        )
        ...
        if metric_dict["intercept"] >= 0:
            ax.text(
                ...
                0.26,
                "{:.3f}x + {:.3f}".format(
                    metric_dict["slope"], metric_dict["intercept"]
                ),
            ...
            ax.text(
                0.95,
                0.26,
                "{:.3f}x {:.3f}".format(metric_dict["slope"], metric_dict["intercept"]),
                verticalalignment="bottom",
                horizontalalignment="right",
                transform=ax.transAxes,
                fontsize=12,
        ...
        ax.text(
            0.95,
            0.12,
            "$R^2$: {0:.{1}f}".format(metric_dict["r2"], 2),
            verticalalignment="bottom",
            horizontalalignment="right",
            transform=ax.transAxes,
            fontsize=12,
        ...
        ax.text(
            0.95,
            0.05,
            "RMSE: {0:.{1}f}".format(metric_dict["rmse"], 2),
            verticalalignment="bottom",
            horizontalalignment="right",
            transform=ax.transAxes,
            fontsize=12,
...
def coarsen_numpy(x, block_h=16, block_w=20):
    ...
    assert h % block_h == 0 and w % block_w == 0, (
        "Dimensions must be divisible by block size"
    )
...
VALID = read_variable_to_vector(
    CARDAMOM_DRIVER_DATA_DIR, "era_valid_v6.nc", "era_valid"
)
INVALID = (
    np.isnan(RUN_SIMULATION_IDX) | np.invert(VALID) | (RUN_SIMULATION_IDX < 0)
)  # filter out dev PIXELS
TEST = np.invert(np.isnan(RUN_SIMULATION_IDX) | np.invert(VALID)) & (
    RUN_SIMULATION_IDX < 0
)

predictor_matrix = read_multiple_varible_to_array(
    CARDAMOM_DRIVER_DATA_DIR, DIFFERLAND_DRIVER_NAME, predictor_list
)
...
scaled_predictor_matrix_dev = (
    predictor_matrix_dev.T - np.mean(predictor_matrix_train, axis=1)
) / np.std(predictor_matrix_train, axis=1)
...
met_matrix_train = jnp.array(
    met_matrix[sorted_valid_idx <= train_dev_idx, :], dtype=jnp.float32
)
met_matrix_dev = jnp.array(
    met_matrix[sorted_valid_idx > train_dev_idx, :], dtype=jnp.float32
)

test_met_matrix = read_multiple_variable_temporal_to_vector(
    CARDAMOM_DRIVER_DATA_DIR,
    DIFFERLAND_DRIVER_NAME,
    met_list,
    test_not_nan_idx,
...
met_matrix_test = jnp.array(
    jnp.transpose(test_met_matrix, axes=[2, 1, 0]), dtype=jnp.float32
)
...
output_matrix_train = jnp.array(
    output_matrix[sorted_valid_idx <= train_dev_idx, :], dtype=jnp.float32
)
output_matrix_dev = jnp.array(
    output_matrix[sorted_valid_idx > train_dev_idx, :], dtype=jnp.float32
)

output_matrix_test = nan_read_multiple_variable_temporal_to_vector(
    CARDAMOM_DRIVER_DATA_DIR,
    DIFFERLAND_DRIVER_NAME,
    output_list,
    test_not_nan_idx,
...
output_matrix_test = output_matrix_test.at[
    np.invert(TEST_ASSIMILATE_SHUFFLE_FLAG), :, 2
].set(-9999)
output_matrix_test = output_matrix_test.at[
    np.invert(TEST_ASSIMILATE_SHUFFLE_FLAG), :, 3
].set(0)
output_matrix_test = output_matrix_test.at[
    np.invert(TEST_ASSIMILATE_SHUFFLE_FLAG), :, 6
].set(-9999)
output_matrix_test = output_matrix_test.at[
    np.invert(TEST_ASSIMILATE_SHUFFLE_FLAG), :, 7
].set(0)
...
predicted_matrix_train = compute_forward_run(
    pickle_name, train_matrix, met_matrix_train
)
...
EPS = 1e-8  # for safe divisions


# ---------- run all three splits (water_thr = 0.7 everywhere) ----------
...
(
    ...
    param_state, train_X, train_Y, train_MET, warm_up, model, water_thr=0.7
)
...
(
    ...
    fire_predicted_test,
    fire_label_test,
) = process_split(param_state, test_X, test_Y, test_MET, warm_up, model, water_thr=0.7)
...
(
    ...
    fire_predicted_dev,
    fire_label_dev,
) = process_split(param_state, dev_X, dev_Y, dev_MET, warm_up, model, water_thr=0.7)

# train
modeled_agb_train, observed_agb_train, agb_mask_train = compute_agb(
    predicted_matrix_train, output_matrix_train, warm_up, model.pfn
)
modeled_agb_train_annual = annualize_monthly(modeled_agb_train)
observed_agb_train_annual = annualize_monthly(observed_agb_train)
agb_train_mask_annual = annualize_mask(agb_mask_train)

# test
modeled_agb_test, observed_agb_test, agb_mask_test = compute_agb(
    predicted_matrix_test, output_matrix_test, warm_up, model.pfn
)
modeled_agb_test_annual = annualize_monthly(modeled_agb_test)
observed_agb_test_annual = annualize_monthly(observed_agb_test)
agb_test_mask_annual = annualize_mask(agb_mask_test)

# dev
modeled_agb_dev, observed_agb_dev, agb_mask_dev = compute_agb(
    predicted_matrix_dev, output_matrix_dev, warm_up, model.pfn
)
modeled_agb_dev_annual = annualize_monthly(modeled_agb_dev)
observed_agb_dev_annual = annualize_monthly(observed_agb_dev)
agb_dev_mask_annual = annualize_mask(agb_mask_dev)


result_dict = {}

compute_metrics(
    np.array(output_matrix_train[:, warm_up:, 4]),
...
compute_metrics(
    np.array(output_matrix_train[:, warm_up:, 14]),
...
compute_metrics(
    np.array(output_matrix_train[:, warm_up:, 0]),
...
compute_metrics(
    np.array(observed_agb_train_annual),
...
compute_metrics(
    np.array(output_matrix_train[:, warm_up:, 22]),
...
compute_metrics(
    np.array(output_matrix_test[:, warm_up:, 4]),
...
compute_metrics(
    np.array(output_matrix_test[:, warm_up:, 14]),
...
compute_metrics(
    np.array(output_matrix_test[:, warm_up:, 0]),
...
compute_metrics(
    np.array(observed_agb_test_annual),
...
compute_metrics(
    np.array(output_matrix_test[:, warm_up:, 22]),
...
compute_metrics(
    np.array(output_matrix_dev[:, warm_up:, 4]),
...
compute_metrics(
    np.array(output_matrix_dev[:, warm_up:, 14]),
...
compute_metrics(
    np.array(output_matrix_dev[:, warm_up:, 22]),
...
compute_metrics(
    np.array(output_matrix_dev[:, warm_up:, 0]),
...
compute_metrics(
    np.array(observed_agb_dev_annual),
...
if args.verbose:
    print(
        "Model performance statistics saved to {}".format(
            os.path.join(METRICS_DIR, "{}_metrics.json".format(exp_str))
        )
    )
...
ax = axs.flatten()
plot_performance(
    predicted_matrix_dev[:, warm_up:, model.pfn.lai].flatten(),
    output_matrix_dev[:, warm_up:, 4].flatten(),
    ax[0],
    0,
    7.5,
    "LAI (m$^2$ m$^{-2}$)",
...
plot_performance(
    predicted_matrix_dev[:, warm_up:, model.pfn.SIF].flatten(),
    output_matrix_dev[:, warm_up:, 0].flatten(),
    ax[1],
    0,
    0.75,
...
plot_performance(
    nbe_predicted_dev.flatten(),
    nbe_label_dev.flatten(),
    ax[2],
    -5,
    5,
    "NBE (gC m$^{-2}$ day $^{-1}$)",
...
plot_performance(
    water_predicted_dev.flatten(),
    water_label_dev.flatten(),
    ax[3],
    -500,
    500,
    "Water Anomaly " + "(kg H$_{2}$O m$^{-2}$)",
)
plot_performance(
    predicted_matrix_dev[:, warm_up:, model.pfn.vod].flatten(),
    output_matrix_dev[:, warm_up:, 14].flatten(),
    ax[4],
    0,
    1.4,
    "VOD",
...
plot_performance(
    modeled_agb_dev_annual.flatten(),
    observed_agb_dev_annual.flatten(),
    ax[5],
    0,
    50000,
    "Live Biomass (gC m$^{-2}$)",
...
plot_performance(
    predicted_matrix_dev[:, warm_up:, model.pfn.ET].flatten(),
    output_matrix_dev[:, warm_up:, 22].flatten(),
    ax[6],
    0,
    7,
...
plot_performance(
    fire_predicted_dev.flatten(),
    fire_label_dev.flatten(),
    ax[7],
    0,
    2.5,
    "Fire C Emission (gC m$^{-2}$ day #$^{-1}$)",
)
...
ax = axs.flatten()
plot_performance(
    predicted_matrix_train[:, warm_up:, model.pfn.lai].flatten(),
    output_matrix_train[:, warm_up:, 4].flatten(),
    ax[0],
    0,
    7.5,
    "LAI (m$^2$ m$^{-2}$)",
...
plot_performance(
    predicted_matrix_train[:, warm_up:, model.pfn.SIF].flatten(),
    output_matrix_train[:, warm_up:, 0].flatten(),
    ax[1],
    0,
    0.75,
    "SIF (mW m$^{-2}$ nm$^{-1}$ sr$^{-1}$)",
...
plot_performance(
    nbe_predicted_train.flatten(),
    nbe_label_train.flatten(),
    ax[2],
    -5,
    5,
    "NBE (gC m$^{-2}$ day $^{-1}$)",
...
plot_performance(
    water_predicted_train.flatten(),
    water_label_train.flatten(),
    ax[3],
    -500,
    500,
    "Water Anomaly " + "(kg H$_{2}$O m$^{-2}$)",
...
plot_performance(
    predicted_matrix_train[:, warm_up:, model.pfn.vod].flatten(),
    output_matrix_train[:, warm_up:, 14].flatten(),
    ax[4],
    0,
    1.4,
    "VOD",
...
plot_performance(
    modeled_agb_train_annual.flatten(),
    observed_agb_train_annual.flatten(),
    ax[5],
    0,
    50000,
    "Live Biomass (gC m$^{-2}$)",
...
plot_performance(
    predicted_matrix_train[:, warm_up:, model.pfn.ET].flatten(),
    output_matrix_train[:, warm_up:, 22].flatten(),
    ax[6],
    0,
    7,
    "Evapotranspiration (mm day $^{-1}$)",
...
plot_performance(
    fire_predicted_train.flatten(),
    fire_label_train.flatten(),
    ax[7],
    0,
    2.5,
    "Fire C Emission (gC m$^{-2}$ day #$^{-1}$)",
...
ax = axs.flatten()
plot_performance(
    predicted_matrix_test[:, warm_up:, model.pfn.lai].flatten(),
    output_matrix_test[:, warm_up:, 4].flatten(),
    ax[0],
    0,
    7.5,
    "LAI (m$^2$ m$^{-2}$)",
...
plot_performance(
    predicted_matrix_test[:, warm_up:, model.pfn.SIF].flatten(),
    output_matrix_test[:, warm_up:, 0].flatten(),
    ax[1],
    0,
    0.75,
    "SIF (mW m$^{-2}$ nm$^{-1}$ sr$^{-1}$)",
...
plot_performance(
    nbe_predicted_test.flatten(),
    nbe_label_test.flatten(),
    ax[2],
    -5,
    5,
    "NBE (gC m$^{-2}$ day $^{-1}$)",
...
plot_performance(
    water_predicted_test.flatten(),
    water_label_test.flatten(),
    ax[3],
    -500,
    500,
    "Water Anomaly " + "(kg H$_{2}$O m$^{-2}$)",
...
plot_performance(
    predicted_matrix_test[:, warm_up:, model.pfn.vod].flatten(),
    output_matrix_test[:, warm_up:, 14].flatten(),
    ax[4],
    0,
    1.4,
    "VOD",
...
plot_performance(
    modeled_agb_test_annual.flatten(),
    observed_agb_test_annual.flatten(),
    ax[5],
    0,
    50000,
    "Live Biomass (gC m$^{-2}$)",
...
plot_performance(
    predicted_matrix_test[:, warm_up:, model.pfn.ET].flatten(),
    output_matrix_test[:, warm_up:, 22].flatten(),
    ax[6],
    0,
    7,
    "Evapotranspiration (mm day $^{-1}$)",
...
plot_performance(
    fire_predicted_test.flatten(),
    fire_label_test.flatten(),
    ax[7],
    0,
    2.5,
    "Fire C Emission (gC m$^{-2}$ day #$^{-1}$)",
...
predictor_matrix_all = read_multiple_varible_to_array(
    CARDAMOM_DRIVER_DATA_DIR, DIFFERLAND_DRIVER_NAME, predictor_list
)
...
scaled_predictor_matrix_all = (
    predictor_matrix_all.T - np.mean(predictor_matrix_train, axis=1)
) / np.std(predictor_matrix_train, axis=1)
...
met_matrix_all = read_multiple_variable_temporal_to_vector(
    CARDAMOM_DRIVER_DATA_DIR,
    DIFFERLAND_DRIVER_NAME,
    met_list,
    np.full(predictor_matrix_all.shape[1], True),
    np.arange(0, predictor_matrix_all.shape[1]),
    n_t=NT,
)
...
embedded_layer_all, dalec_parameters_all, pheno_parameters_all, initial_pools_all = (
    batch_param_forward(param_state, all_matrix, met_matrix_all)
)
embedded_layer_all = embedded_layer_all.reshape(720, 1440, embedded_layer_all.shape[-1])
dalec_parameters_all = dalec_parameters_all.reshape(
    720, 1440, dalec_parameters_all.shape[-1]
)
pheno_parameters_all = pheno_parameters_all.reshape(
    720, 1440, pheno_parameters_all.shape[-1]
)
...
for i in range(len(dalec_parnames)):
    ds[dalec_parnames[i]] = xr.DataArray(
        np.expand_dims(dalec_parameters_all[:, :, i], axis=0),
        coords={
            "run": [
                run,
            ],
...
for i in range(len(pheno_parnames)):
    ds[pheno_parnames[i]] = xr.DataArray(
        np.expand_dims(pheno_parameters_all[:, :, i], axis=0),
        coords={
            "run": [
                run,
            ],
...
for i in range(len(initial_poolnames)):
    ds[initial_poolnames[i]] = xr.DataArray(
        np.expand_dims(initial_pools_all[:, :, i], axis=0),
        coords={
            "run": [
                run,
            ],
...
observed_lai_da = xr.open_dataset(
    os.path.join(CARDAMOM_DRIVER_DATA_DIR, DIFFERLAND_DRIVER_NAME)
)["LAI"][warm_up:, :, :]
predicted_lai_da = xr.DataArray(
    predicted_all[:, :, warm_up:, model.pfn.lai].transpose([2, 0, 1]),
    coords={
        "time": observed_lai_da.time,
        "lat": np.linspace(89.875, -89.875, 720),
        "lon": np.linspace(-179.875, 179.875, 1440),
    },
)

cov_lai, cor_lai, slope_lai, intercept_lai, pval_lai, stderr_lai = lag_linregress_3D(
...
(
    ...
    cor_lai_annual,
    slope_lai_annual,
    intercept_lai_annual,
    pval_lai_annual,
    stderr_lai_annual,
) = lag_linregress_3D(predicted_lai_annual_da, observed_lai_annual_da)
...
(
    ...
    pval_lai_anom,
    stderr_lai_anom,
) = lag_linregress_3D(predicted_lai_anom_da, observed_lai_anom_da)


observed_sif_da = xr.open_dataset(
    os.path.join(CARDAMOM_DRIVER_DATA_DIR, DIFFERLAND_DRIVER_NAME)
)["SIF"][warm_up:, :, :]
predicted_sif_da = xr.DataArray(
    predicted_all[:, :, warm_up:, model.pfn.SIF].transpose([2, 0, 1]),
    coords={
        "time": observed_sif_da.time,
        "lat": np.linspace(89.875, -89.875, 720),
        "lon": np.linspace(-179.875, 179.875, 1440),
    },
)
...
(
    ...
    cor_sif_annual,
    slope_sif_annual,
    intercept_sif_annual,
    pval_sif_annual,
    stderr_sif_annual,
) = lag_linregress_3D(predicted_sif_annual_da, observed_sif_annual_da)
...
(
    ...
    pval_sif_anom,
    stderr_sif_anom,
) = lag_linregress_3D(predicted_sif_anom_da, observed_sif_anom_da)

observed_et_da = xr.open_dataset(
    os.path.join(CARDAMOM_DRIVER_DATA_DIR, DIFFERLAND_DRIVER_NAME)
)["ET"][warm_up:, :, :]

predicted_et_da = xr.DataArray(
    predicted_all[:, :, warm_up:, model.pfn.ET].transpose([2, 0, 1]),
    coords={
        "time": observed_et_da.time,
        "lat": np.linspace(89.875, -89.875, 720),
        "lon": np.linspace(-179.875, 179.875, 1440),
    },
)
...
(
    ...
    pval_et_annual,
    stderr_et_annual,
) = lag_linregress_3D(predicted_et_annual_da, observed_et_annual_da)
...
(
    ...
    pval_et_anom,
    stderr_et_anom,
) = lag_linregress_3D(predicted_et_anom_da, observed_et_anom_da)


observed_nbe_da = xr.open_dataset(
    os.path.join(CARDAMOM_DRIVER_DATA_DIR, DIFFERLAND_DRIVER_NAME)
)["NBE"][warm_up:, :, :]
observed_nbe_da.values = coarsen_numpy(observed_nbe_da.values)
predicted_nbe_da = xr.DataArray(
    predicted_all[:, :, warm_up:, model.pfn.nbe].transpose([2, 0, 1]),
    coords={
        "time": observed_nbe_da.time,
        "lat": np.linspace(89.875, -89.875, 720),
        "lon": np.linspace(-179.875, 179.875, 1440),
    },
)
...
(
    ...
    cor_nbe_annual,
    slope_nbe_annual,
    intercept_nbe_annual,
    pval_nbe_annual,
    stderr_nbe_annual,
) = lag_linregress_3D(predicted_nbe_annual_da, observed_nbe_annual_da)

observed_ewt_da = xr.open_dataset(
    os.path.join(CARDAMOM_DRIVER_DATA_DIR, DIFFERLAND_DRIVER_NAME)
)["LWE_normalized"][warm_up:, :, :]
predicted_ewt_da = xr.DataArray(
    predicted_all[:, :, warm_up:, model.pfn.next_water_pool].transpose([2, 0, 1]),
    coords={
        "time": observed_ewt_da.time,
        "lat": np.linspace(89.875, -89.875, 720),
        "lon": np.linspace(-179.875, 179.875, 1440),
    },
)
...
predicted_biomass_da = xr.DataArray(
    (
        ...
        + predicted_all[:, :, warm_up:, model.pfn.next_wood_pool]
        + predicted_all[:, :, warm_up:, model.pfn.next_labile_pool]
        + predicted_all[:, :, warm_up:, model.pfn.next_root_pool]
    ).transpose([2, 0, 1]),
    coords={
        "time": observed_biomass_da.time,
        "lat": np.linspace(89.875, -89.875, 720),
        "lon": np.linspace(-179.875, 179.875, 1440),
    },
)
...
(
    cov_biomass_annual,
    cor_biomass_annual,
    slope_biomass_annual,
    intercept_biomass_annual,
    pval_biomass_annual,
    stderr_biomass_annual,
) = lag_linregress_3D(predicted_biomass_annual_da, observed_biomass_annual_da)

rmse_lai = vectorized_nan_filtered_root_mean_squared_error(
    predicted_lai_da, observed_lai_da
)
rmse_sif = vectorized_nan_filtered_root_mean_squared_error(
    predicted_sif_da, observed_sif_da
)
rmse_et = vectorized_nan_filtered_root_mean_squared_error(
    predicted_et_da, observed_et_da
)
rmse_nbe = vectorized_nan_filtered_root_mean_squared_error(
    predicted_nbe_da, observed_nbe_da
)
rmse_ewt = vectorized_nan_filtered_root_mean_squared_error(
    predicted_ewt_da, observed_ewt_da
)
rmse_biomass = vectorized_nan_filtered_root_mean_squared_error(
    predicted_biomass_da, observed_biomass_da
)
...
metrics_ds["cor_nbe"] = cor_nbe.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["cor_sif"] = cor_sif.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["cor_lai"] = cor_lai.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["cor_ewt"] = cor_ewt.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["cor_et"] = cor_et.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["cor_biomass"] = cor_biomass.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["cor_nbe_annual"] = cor_nbe_annual.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["cor_sif_annual"] = cor_sif_annual.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["cor_lai_annual"] = cor_lai_annual.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["cor_ewt_annual"] = cor_ewt_annual.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["cor_et_annual"] = cor_et_annual.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["cor_biomass_annual"] = cor_biomass_annual.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["cor_sif_anom"] = cor_sif_annual.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["cor_lai_anom"] = cor_lai_annual.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["cor_et_anom"] = cor_et_anom.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["rmse_nbe"] = rmse_nbe.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["rmse_sif"] = rmse_sif.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["rmse_lai"] = rmse_lai.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["rmse_ewt"] = rmse_ewt.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["rmse_et"] = rmse_et.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["rmse_biomass"] = rmse_biomass.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["r2_nbe"] = r2_nbe.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["r2_sif"] = r2_sif.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["r2_lai"] = r2_lai.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["r2_ewt"] = r2_ewt.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["r2_et"] = r2_et.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["r2_biomass"] = r2_biomass.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["trend_nbe"] = trend_nbe.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["trend_sif"] = trend_sif.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["trend_lai"] = trend_lai.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["trend_ewt"] = trend_ewt.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["trend_et"] = trend_et.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["trend_biomass"] = trend_biomass.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["trend_p_nbe"] = trend_p_nbe.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["trend_p_sif"] = trend_p_sif.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["trend_p_lai"] = trend_p_lai.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["trend_p_ewt"] = trend_p_ewt.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["trend_p_et"] = trend_p_et.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["trend_p_biomass"] = trend_p_biomass.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds = xr.Dataset()
metrics_ds["slope_nbe"] = slope_nbe.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["slope_sif"] = slope_sif.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["slope_lai"] = slope_lai.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["slope_ewt"] = slope_ewt.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["slope_et"] = slope_et.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["intercept_nbe"] = intercept_nbe.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["intercept_sif"] = intercept_sif.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["intercept_lai"] = intercept_lai.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["intercept_ewt"] = intercept_ewt.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["intercept_et"] = intercept_et.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["pval_nbe"] = pval_nbe.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["pval_sif"] = pval_sif.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["pval_lai"] = pval_lai.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["pval_ewt"] = pval_ewt.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["pval_et"] = pval_et.expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["nbe_mean"] = np.mean(predicted_nbe_da, axis=0).expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["sif_mean"] = np.mean(predicted_sif_da, axis=0).expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["ewt_mean"] = np.mean(predicted_ewt_da, axis=0).expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["lai_mean"] = np.mean(predicted_lai_da, axis=0).expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["et_mean"] = np.mean(predicted_et_da, axis=0).expand_dims(
    dim={
        "run": [
            run,
        ]
...
predicted_beta_da = xr.DataArray(
    predicted_all[:, :, warm_up:, model.pfn.beta].transpose([2, 0, 1]),
    coords={
        "time": observed_sif_da.time,
        "lat": np.linspace(89.875, -89.875, 720),
        "lon": np.linspace(-179.875, 179.875, 1440),
...
metrics_ds["beta_mean"] = np.mean(predicted_beta_da, axis=0).expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["beta_std"] = np.std(predicted_beta_da, axis=0).expand_dims(
    dim={
        "run": [
            run,
        ]
...
predicted_labile_da = xr.DataArray(
    predicted_all[:, :, warm_up:, model.pfn.next_labile_pool].transpose([2, 0, 1]),
    coords={
        "time": observed_ewt_da.time,
        "lat": np.linspace(89.875, -89.875, 720),
        "lon": np.linspace(-179.875, 179.875, 1440),
...
metrics_ds["labile_mean"] = np.mean(predicted_labile_da, axis=0).expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["labile_initial"] = predicted_labile_da[0, :, :].expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["labile_final"] = predicted_labile_da[-12, :, :].expand_dims(
    dim={
        "run": [
            run,
        ]
...
predicted_foliar_da = xr.DataArray(
    predicted_all[:, :, warm_up:, model.pfn.next_foliar_pool].transpose([2, 0, 1]),
    coords={
        "time": observed_ewt_da.time,
        "lat": np.linspace(89.875, -89.875, 720),
        "lon": np.linspace(-179.875, 179.875, 1440),
...
metrics_ds["foliar_mean"] = np.mean(predicted_foliar_da, axis=0).expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["foliar_initial"] = predicted_foliar_da[0, :, :].expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["foliar_final"] = predicted_foliar_da[-12, :, :].expand_dims(
    dim={
        "run": [
            run,
        ]
...
predicted_wood_da = xr.DataArray(
    predicted_all[:, :, warm_up:, model.pfn.next_wood_pool].transpose([2, 0, 1]),
    coords={
        "time": observed_ewt_da.time,
        "lat": np.linspace(89.875, -89.875, 720),
        "lon": np.linspace(-179.875, 179.875, 1440),
...
metrics_ds["wood_mean"] = np.mean(predicted_wood_da, axis=0).expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["wood_initial"] = predicted_wood_da[0, :, :].expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["wood_final"] = predicted_wood_da[-12, :, :].expand_dims(
    dim={
        "run": [
            run,
        ]
...
predicted_root_da = xr.DataArray(
    predicted_all[:, :, warm_up:, model.pfn.next_root_pool].transpose([2, 0, 1]),
    coords={
        "time": observed_ewt_da.time,
        "lat": np.linspace(89.875, -89.875, 720),
        "lon": np.linspace(-179.875, 179.875, 1440),
...
metrics_ds["root_mean"] = np.mean(predicted_root_da, axis=0).expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["root_initial"] = predicted_root_da[0, :, :].expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["root_final"] = predicted_root_da[-12, :, :].expand_dims(
    dim={
        "run": [
            run,
        ]
...
predicted_litter_da = xr.DataArray(
    predicted_all[:, :, warm_up:, model.pfn.next_litter_pool].transpose([2, 0, 1]),
    coords={
        "time": observed_ewt_da.time,
        "lat": np.linspace(89.875, -89.875, 720),
        "lon": np.linspace(-179.875, 179.875, 1440),
...
metrics_ds["litter_mean"] = np.mean(predicted_litter_da, axis=0).expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["litter_initial"] = predicted_litter_da[0, :, :].expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["litter_final"] = predicted_litter_da[-12, :, :].expand_dims(
    dim={
        "run": [
            run,
        ]
...
predicted_som_da = xr.DataArray(
    predicted_all[:, :, warm_up:, model.pfn.next_som_pool].transpose([2, 0, 1]),
    coords={
        "time": observed_ewt_da.time,
        "lat": np.linspace(89.875, -89.875, 720),
        "lon": np.linspace(-179.875, 179.875, 1440),
...
metrics_ds["som_mean"] = np.mean(predicted_som_da, axis=0).expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["som_initial"] = predicted_som_da[0, :, :].expand_dims(
    dim={
        "run": [
            run,
        ]
...
metrics_ds["som_final"] = predicted_som_da[-12, :, :].expand_dims(
    dim={
        "run": [
            run,
        ]
...
if args.verbose:
    print(
        "Mean and std of select pools saved to {}".format(
            os.path.join(NC_DIR, "{}_mean_std.nc".format(exp_str))
        )
    )
...
nbe_ds = xr.Dataset(
    {
        "predicted_nbe": predicted_nbe_da.expand_dims(
            dim={
                "run": [
                    run,
                ]
...
if args.verbose:
    print(
        "Modeled NBE saved to {}".format(
            os.path.join(NC_DIR, "{}_nbe.nc".format(exp_str))
        )
    )


predicted_fire_da = xr.DataArray(
    predicted_all[:, :, warm_up:, model.pfn.total_fire_combust].transpose([2, 0, 1]),
    coords={
        "time": observed_nbe_da.time,
        "lat": np.linspace(89.875, -89.875, 720),
        "lon": np.linspace(-179.875, 179.875, 1440),
...
predicted_gpp_da = xr.DataArray(
    predicted_all[:, :, warm_up:, model.pfn.gpp].transpose([2, 0, 1]),
    coords={
        "time": observed_nbe_da.time,
        "lat": np.linspace(89.875, -89.875, 720),
        "lon": np.linspace(-179.875, 179.875, 1440),
...
gpp_et_ds = xr.Dataset(
    {
        "predicted_gpp": predicted_gpp_da.expand_dims(
            dim={
                "run": [
                    run,
                ]
        ...
        "predicted_et": predicted_et_da.expand_dims(
            dim={
                "run": [
                    run,
                ]
        ...
        "predicted_fire": predicted_fire_da.expand_dims(
            dim={
                "run": [
                    run,
                ]
...
if args.verbose:
    print(
        "Modeled GPP & ET saved to {}".format(
            os.path.join(NC_DIR, "{}_gpp_et.nc".format(exp_str))
        )
    )
...
if args.verbose:
    print(
        "Spatial embeding saved to {}".format(
            os.path.join(NPY_DIR, "{}_embed.npy".format(exp_str))
        )
    )
